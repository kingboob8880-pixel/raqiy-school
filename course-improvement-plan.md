...
## Десятый проход (2026-07-20) — редизайн "Пути ученика" + сайтовый backlog контраста

По запросу "переработать вид, чтобы было красиво" обновлён внешний вид
`pages/modules/index.html` (hover-lift карточек, заливка маркера, стрелка-
подсказка, вынесенный статус, лёгкое проявление при прокрутке, поле поиска
в переиспользуемый `.search-field`). Подробности — `project.md`, запись
"Редизайн «Путь ученика»…" (2026-07-20).

**Backlog-находка (не исправлено самовольно):** `--rp-learn-text-muted` на
фоне `--rp-learn-surface` даёт **3.78:1 на baseline/светлой теме** — ниже
WCAG AA 4.5:1. Используется в ~12 местах: `.feature-item p`,
`.advantage-card p`, `.stats-strip__label`, `.process-step p`,
`.author-card__role`, `.faq-item p`, `.path-step__desc`, `.audience-card p`,
`.video-showcase p`, `.skeleton-loading`, `.footer` — почти все на лендинге,
уже принятом автором визуально. Точечно исправил только там, где сам
редактировал в этом проходе (`.path-step__meta`, через уже проверенный
паттерн `var(--rp-zone-text)+opacity:0.7`). Остальные 11 мест — не трогал:
смена глобального токена `--rp-learn-text-muted` изменила бы визуальный вес
текста на уже одобренных карточках лендинга без разрешения автора на такую
правку. Нужно отдельное решение: либо чуть затемнить сам токен (сузит
контраст-гэп на всех 3 темах разом), либо переводить каждое место по
отдельности на `var(--rp-zone-text)+opacity` (уже проверенный паттерн) —
на усмотрение автора, следующий проход.

Изменено: см. `project.md`. Не реализовано (сознательно, см. выше):
исправление `--rp-learn-text-muted` в оставшихся ~11 местах.
