...
exports.telegramWebhook = onRequest({ region: REGION, secrets: SECRETS }, async (req, res) => {
  ...
  try {
    if (u.callback_query) {
      ...
      //
      // Сверяем ЕЩЁ И НАЖАВШЕГО (from.id), а не только чат. Пока TG_CHAT_ID
      // — личный чат автора, разницы нет: в личном чате нажать может только
      // он. Но стоит однажды поставить туда id группы (например, чтобы
      // помогал ассистент) — и админом станет любой участник, нажавший
      // кнопку в этой группе, включая случайно добавленного. Проверка
      // стоит копейки и снимает эту мину заранее.
      const pressedBy = String(u.callback_query.from?.id ?? "");
      if (from === CHAT && pressedBy === CHAT) {
