...
/** Открыть/закрыть ученику весь курс сразу — кнопка в кабинете админа
 *  (запрос автора 2026-07-27: «дать доступ ко всем модулям и упражнениям
 *  тем ученикам, которых я выберу»).
 *
 *  Что снимает поле fullAccess:
 *    • порядок модулей — открыты все одиннадцать, а не только следующий
 *      за сданным;
 *    • замок на упражнениях — не нужно сдавать экзамен книги;
 *    • пейвол — полный текст книг открыт, иначе «доступ ко всем модулям»
 *      означал бы одиннадцать страниц с урезанными отрывками.
 *
 *  Почему отдельным полем, а не через paid. Оплата — факт о деньгах, по
 *  ней автор считает статистику и решает вопросы возврата. Выданный
 *  вручную доступ — решение автора, и смешивать их значит навсегда
 *  потерять ответ на вопрос «сколько человек реально оплатили».
 *
 *  ВНИМАНИЕ: пейвол защищён и правилами Firestore, не только клиентом.
 *  Пока integration/firestore.rules не задеплоены, полный текст такому
 *  ученику не откроется — модули и упражнения откроются сразу. */
export async function setFullAccess(uid, granted) {
  const update = { fullAccess: granted };
  if (granted) update.fullAccessAt = serverTimestamp();
  await updateDoc(doc(db, "students", uid), update);
}
