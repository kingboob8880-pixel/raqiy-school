...
Good "firebase $((& firebase --version) -split "`n" | Select-Object -First 1)"
...
foreach ($key in @("TG_BOT_TOKEN", "TG_CHAT_ID", "TG_WEBHOOK_SECRET")) {
    ...
    try {
        $out = & firebase functions:secrets:set $key --data-file $tmp --project $PROJECT --force 2>&1 | Out-String
...
Push-Location $ROOT
$out = & node scripts/build-course-data.mjs 2>&1 | Out-String
$out | Out-File $LOG -Append -Encoding UTF8
if ($LASTEXITCODE -ne 0) { Pop-Location; Say $out "DarkGray"; Fail "Не собрались данные курса" }
Good ($out.Trim() -split "`n" | Select-Object -First 1)

# ─────────────────────────────────────────────────────────────────────────
Step 6 "Выкладываю функции — это 2-5 минут, окно не закрывайте"

& firebase deploy --only functions --project $PROJECT 2>&1 | Tee-Object -FilePath $LOG -Append
if ($LASTEXITCODE -ne 0) { Pop-Location; Fail "Деплой функций не прошёл" "Ошибка видна выше и записана в лог." }
Good "Функции выложены"

Step 7 "Выкладываю правила доступа"
& firebase deploy --only firestore:rules --project $PROJECT 2>&1 | Tee-Object -FilePath $LOG -Append
if ($LASTEXITCODE -ne 0) { Pop-Location; Fail "Правила не выложились" }
